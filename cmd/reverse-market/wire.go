...
func initApp() (*Application, func(), error) {
	wire.Build(
		getConfig,
		log.NewLoggers,
		wire.Bind(new(loggers), new(*log.Loggers)),
		wire.Bind(new(redis.Loggers), new(*log.Loggers)),
		NewJwtManager,
		wire.Bind(new(tokensManager), new(*jwt.Manager)),
		NewRedisConfig,
		redis.NewRedisPool,
		redis.NewRefreshTokenRepository,
		wire.Bind(new(refreshTokensRepository), new(*redis.RefreshTokenRepository)),
		NewPostgresConfig,
